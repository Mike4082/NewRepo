...
CLIENT_ID = "ff0b347da8affc439353"
CLIENT_SECRET = "27ca55f35513d7c0641290d1f44f5ec9660d4025"
...
def login():
    github_auth_url = (
        f"https://github.com/login/oauth/authorize"         f"?client_id={CLIENT_ID}"
        f"&redirect_uri=http://localhost:8000/auth/callback"         f"&scope=repo"     )     
    return RedirectResponse(url=github_auth_url)
@app.get("/auth/callback") 
async def auth_callback(code: str):
    token_url = "https://github.com/login/oauth/access_token"     
    async with httpx.AsyncClient() as client:         
        token_response = await client.post(token_url, headers={"Accept": "application/json"},             
    ...
    user_data[username] = {"email": user_data_response.get("email"), "repos": [],"token": access_token} 
...
async def add_repository(username: str, repo_data: RepoData):
    ...
    data = repo_data.dict()     
    async with httpx.AsyncClient() as client:
        response = await client.post(             "https://api.github.com/user/repos",             headers=headers,             json=data         )
...
async def add_webhook(username: str, repo_name: str = Query(...)):
    ...
    webhook_url = "http://localhost:8000/repo/Final-Project?username=Mike4082/webhook_receiver"  
    ...
    data = {         "name": "web",
        "active": True,         "events": ["push", "pull_request", "issue_comment"],         "config": webhook_config
    }
    ...
    async with httpx.AsyncClient() as client:
        response = await client.post(             f"https://api.github.com/repos/{username}/{repo_name}/hooks",             headers=headers,             json=data         )
    # Check the response status     if response.status_code != 201:
    raise HTTPException(status_code=response.status_code, detail=response.text)
        # Return a success message and webhook details
    return {"message": "Webhook added successfully", "webhook_info":response.json()}
